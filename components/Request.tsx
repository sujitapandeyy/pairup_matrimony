...
const Requests = () => {
    ...
    try {
      ...
      let filtered = res.data.filter(
        (n: any) =>
          ...
          n.to?.toLowerCase() === userEmail.toLowerCase()
      );
  ...
  useEffect(() => {
    if (userId) fetchCurrentUserProfile(userId);
  }, [userId]);
  ...
  const handleAccept = async (request: any, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.success(
        isMatch
          ? `It's a Match with ${request.sender_name}!`
          : `You accepted ${request.sender_name}'s request!`
      );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">
        Match Requests and Notifications
      </h1>
              ...
              >
                <div className="p-4 flex items-start gap-3">
                    ...
                    {request.type === "request" &&
                      (request.compatibility_score || request.score) && (
                        ...
                        >
                          ...
                          request.compatibility_score === "Not Compatible"
                            ? "Not Compatible"
                            : `${Math.round(
                                request.compatibility_score || request.score
                              )}% Compatible`}
                  ...
                  <div className="flex-shrink-0 flex items-start gap-1 text-xs text-gray-400">
                    ...
                    <span>
                      {getTimeAgoKathmandu(request.created_at || new Date())}
                    </span>
                  </div>
                </div>
              </div>
