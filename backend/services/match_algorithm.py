...
class MatchAlgorithm:
    ...
    def get_similar_profiles_for_profile(self, request, profile_email):
        ...
        detail = self._get_document(self.details, profile_user["_id"])
        interests = self._get_document(self.interests, profile_user["_id"])
        interests_list = self._get_interests_list(interests, detail)
        location = self._get_location(detail)
        gender_pref = self._get_gender_preference(interests)

        similar_profiles = []
        for other in self.users.find():
            if other.get("email") == profile_email:
                continue

            profile = self._process_candidate(
                request, other,
                partner_gender_pref=gender_pref,
                liked_emails=[], liked_by_emails=[],
                user_detail=detail,
                user_interests_list=interests_list,
        ...
        return {"similar_profiles": similar_profiles[:3]}
